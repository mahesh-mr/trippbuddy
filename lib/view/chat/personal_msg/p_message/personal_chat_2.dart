...
class PeronalChats2 extends StatelessWidget {
  PeronalChats2({Key? key, required this.allchats, required this.chatindex})
      : super(key: key);
  int chatindex;
SerchChat allchats;
...
//  var msgs = false.obs;
  ...
  Widget build(BuildContext context) {
    ...
    final listChat =postchatController.chats.value[chatindex];

    // String userPic = listChat.id != userId
...
AppBar(
          ...
          child: Obx(() {
            ...
            return Row(
              children: [
                ...
                CircleAvatar(
                  backgroundImage: NetworkImage(postchatController.chats.value[chatindex].pic.toString()),
                  maxRadius: 20,
                ),
                const SizedBox(
                  width: 12,
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        postchatController.chats.value[chatindex].name.toString(),
            ...
            FloatingActionButton(
              onPressed: () {
                PersonalChat? prlChat =
                    personalChatController.personalchats[chatindex];
                bool isValid = _fomkey.currentState!.validate();

                if (isValid) {
                  PostChatService.postChats(
                      chatId: prlChat.id!, content: _msgcontroller.text);
  ...
  // Container inputField(PostchatController postchatController) {
  //   return Container(
  //     padding: const EdgeInsets.only(left: 10, bottom: 10, top: 10),
  //     height: 60,
  //     width: double.infinity,
  ...
  //             onPressed: () {
  //               PersonalChat? prlChat =
  //                   personalChatController.[chatindex];
  //               bool isValid = _fomkey.currentState!.validate();
